    ...
    def fetch_institutions(self):
        ...
        with self.client.get("/api/institutions/", catch_response=True) as response:
            if response.status_code == 200:
                ...
                response.failure(
                    f"Failed to fetch institutions: {response.status_code}"
                )
    ...
    def check_status(self):
        ...
        if self.session_id:
            with self.client.get(
                f"/api/queue/entries/{self.session_id}/status/", catch_response=True
            ) as response:
                if response.status_code != 200:
    ...
    def join_queue(self):
        ...
        if self.institution_id and not self.session_id:
            ...
            with self.client.post(
                "/api/queue/join/", json=payload, catch_response=True
            ) as response:
                if response.status_code == 201:
                    data = response.json()
                    self.session_id = data.get("session_id")
                elif response.status_code == 400:
                    # This might happen if the number was already taken
                    # during the stress test. We mark it as a success because
                    # it's a valid business logic error under stress.
                    response.success()
                else:

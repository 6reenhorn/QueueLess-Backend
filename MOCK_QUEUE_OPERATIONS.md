...
- `POST /api/queue/institutions/{institution_id}/simulate-tick/` is admin only and manually advances the queue state; the `queue_worker` management command can also advance the queue automatically.
